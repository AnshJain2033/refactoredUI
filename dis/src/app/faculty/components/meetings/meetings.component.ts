   ...
   let list = [];
    for(let m of data){
      ...
      console.log(meeting);
       ...
       if(meeting.meetingAttendants){
        
        for(let att of meeting.meetingAttendants){
          if(att == this.userId){
            this.filteredList.push(meeting);
            break;
          }
        }
    ...
    'Past Meetings': () => {
      this.meetingsService.getPastMeetings().subscribe({
        next: (res: Meeting[]) => {
          console.log(res);
          this.filteredList = [];
          this.currentList = this.filterData(res);
