...
@Component({
  selector: 'app-faculty-room',
  templateUrl: './faculty-room.component.html',
  styleUrls: ['./faculty-room.component.scss'],
})
export class FacultyRoomComponent implements OnInit {
  ...
  getInfrastructureByType() {
    this.httpService.getRequest('getInfrastructureByType', ['getRooms']).subscribe({
    ...
    switch (event.name) {
      ...
      case 'Edit': {
        let dialogRef = this.dialog.open(InfrastructureDialogComponent, {
          data: {
            type: 'edit',
            data: { id: event.value.id, infraType: 'Faculty Room' },
          },
          disableClose: true,
        });
